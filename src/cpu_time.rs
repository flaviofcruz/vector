...
//!   `CLOCK_THREAD_CPUTIME_ID` on Linux/macOS and a zero no-op on other
//!   platforms.
...
//!   counter for a component (available on Linux and macOS only).
...
/// ```ignore
/// // Some(counter):
/// tokio::spawn(future.cpu_timed(counter).in_current_span())
/// // None:
...
{
    match counter {
        Some(c) => tokio::spawn(future.cpu_timed(c).in_current_span()),
...
/// On other platforms it returns [`Counter::noop()`] — the metric is **not**
/// emitted at all, rather than reporting wall-clock or zero values that would
/// be misleading to compare against supported platforms.
///
...
pub fn register_counter() -> Counter {
    metrics::counter!("component_cpu_usage_ns_total")
}
