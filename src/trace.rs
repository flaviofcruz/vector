...
// These are the spots where we emit vector event logs via INFO logging. This should NOT go to stdout.
// Note: tracing_subscriber::filter::Targets does nested matching, so specifying a parent
// module will match all child modules (e.g. "foo::bar" matches "foo::bar::baz").
static EVENT_LOG_TARGETS: &[&str] = &[
    "vector::internal_events::vector_event",
    "vector_common::internal_event::vector_event",
];
...
pub fn init(color: bool, json: bool, levels: &str, internal_log_rate_limit: u64) {
    ...
    // Additionally, for logs that are printed out, we add an event log filter
    // Otherwise, the internal vector event logs we emit will overwhelm stdout/stderr
    let event_log_filter = EVENT_LOG_TARGETS
        .iter()
        .fold(fmt_filter.clone(), |targets, &target| {
            targets.with_target(target, LevelFilter::OFF)
        });
    ...
    if json {
        ...
        let subscriber = subscriber.with(rate_limited.with_filter(event_log_filter));
