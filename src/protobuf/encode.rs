...
mod tests {
    ...
    fn test_encode_decode_map_all_key_types() {
        ...
        let encoded = encode_proto(&descriptor, input.clone());
        assert!(
            encoded.is_ok(),
            "encode_proto failed: {:?}",
            encoded.unwrap_err()
        ...
        let decoded = parse_proto(&descriptor, encoded.unwrap());
        assert!(
            decoded.is_ok(),
            "parse_proto failed: {:?}",
            decoded.unwrap_err()
        );
        assert_eq!(input, decoded.unwrap(), "round-trip mismatch");
    }

    #[test]
    fn test_reverse_roundtrip_parse_encode_all_key_types() {
        // Reverse round-trip on the config_map.pb fixture (same key-type coverage as
        // test_encode_decode_map_all_key_types): parse_proto → encode_proto → parse_proto.
        let raw_bytes =
        ...
        let parsed = parse_proto(&descriptor, Value::Bytes(Bytes::from(raw_bytes)))
            .expect("parse_proto failed");

        let re_encoded = encode_proto(&descriptor, parsed.clone()).expect("encode_proto failed");

        let re_parsed =
            parse_proto(&descriptor, re_encoded).expect("parse_proto on re-encoded bytes failed");
