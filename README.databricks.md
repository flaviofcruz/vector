...
129. Emit `events_received_total` and `events_delivered_total` counters alongside the existing VEL `info!()` logs in `DeliveryReadEvent::emit` and `VectorSinkDeliveryEvent::emit_count_map`. Labels are bounded for M3: `delivery_event_type`, `event_time_bucket` (hour-rounded UTC), and `topic`. Bucket is read from `value_map` (populated via `DELIVERY_EVENT_LOG_GRANULARITY_FIELDS` once source-side plumbing is added) so delivered counters use the event's receive-time bucket, making completeness ratios `delivered/received` non-oscillating across the receive→deliver gap. Falls back to current hour if the field is not in `granularity_fields`, in which case behavior matches the legacy oscillating ratio.
