...
132. Add a `component_encode_cpu_seconds` histogram recording the per-batch CPU time each sink spends in `encode_events` (serialize + compress), tagged by `component_id` via the component span, so per-sink encode/compression cost (e.g. gzip vs zstd) is directly observable. Declares `libc` as a `cfg(target_os = "linux")` dependency (previously dev-only). https://github.com/databricks-eng/vector/pull/546
