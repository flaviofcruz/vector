...
129. Emit a `delivery_events_total` counter alongside the existing VEL `info!()` logs in `DeliveryReadEvent::emit` and `VectorSinkDeliveryEvent::emit_count_map`. A single metric is split by stage via the `delivery_event_type` label (`VECTOR_SOURCE_READ`, `VECTOR_SINK_UPLOAD_STAGED`, `VECTOR_SINK_UPLOAD_DELIVERED`), in line with how VEL groups delivery events. Other labels: `time_parity` (hour-rounded Unix milliseconds, matching the convention used in woodchuck VRL transforms) and `topic`. Reuses the existing `DELIVERY_EVENT_LOG_GRANULARITY_FIELDS = ["timeParity", "topic", "deliveryMethod"]` plumbing — delivered events get the event's receive-time bucket from `logMetadata.timeParity` set upstream by `file_based_raw_proto_streaming.libsonnet` and `file_based_unstructured_log_daemon_wrapper.libsonnet`. Receive-side topic resolution mirrors `event_logs.libsonnet`'s `ConvertDeliveryInfoLogs`: explicit `source_context.topic`, then a regex-based Lumberjack filename inference covering both active (`<TableNameCamelCase>[LaMigration].pb[.base64][.gz]`) and archived (`<prefix>.<TableNameCamelCase>[LaMigration].pb[.base64][.gz]`) forms, then a source-type-specific fallback (`"sawmill-service-log"` for `kubernetes_logs`, `"lumberjackTopicInfered"` otherwise). `FileServer` carries a `source_type` field so the file and kubernetes_logs sources tag their emissions correctly.
