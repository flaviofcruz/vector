...
129. Emit `events_received_total` and `events_delivered_total` counters alongside the existing VEL `info!()` logs in `DeliveryReadEvent::emit` and `VectorSinkDeliveryEvent::emit_count_map`. Labels are bounded for M3: `delivery_event_type`, `time_parity` (hour-rounded Unix milliseconds, matching the convention used in woodchuck VRL transforms), and `topic`. Reuses the existing `DELIVERY_EVENT_LOG_GRANULARITY_FIELDS = ["timeParity", "topic", "deliveryMethod"]` plumbing — no Phase 2 source-side changes required for the delivered counter, because upstream VRL transforms (`file_based_raw_proto_streaming.libsonnet`, `file_based_unstructured_log_daemon_wrapper.libsonnet`) already set `logMetadata.timeParity` at receive time. As a result, delivered counters increment with the event's receive-time bucket, making the `delivered_total / received_total` ratio non-oscillating across the receive→deliver gap.
